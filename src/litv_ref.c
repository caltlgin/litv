...
litv_newref()
{
    return calloc(1, sizeof(litv_ref));
}
...
static bool
litv_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
litv_book_matches(const litv_book *book, const char *s)
{
    return litv_bookequal(book->name, s, false) ||
        litv_bookequal(book->abbr, s, false) ||
        litv_bookequal(book->name, s, true);
}

static int
litv_book_fromname(const char *s)
{
    for (int i = 0; i < litv_books_length; i++) {
        const litv_book *book = &litv_books[i];
        if (litv_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
litv_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
litv_parseref(litv_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (litv_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = litv_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
