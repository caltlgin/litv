/*
litv: Read the Word of God from your terminal
...
const char *
usage = "usage: litv [flags] [reference...]\n"
    "\n"
    "Flags:\n"
...
main(int argc, char *argv[])
{
    ...
    for (int opt; (opt = getopt(argc, argv, "A:B:CeplWh")) != -1; ) {
        ...
        case 'A':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "litv: invalid flag value for -A\n\n%s", usage);
                return 1;
            }
            break;
        case 'B':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "litv: invalid flag value for -B\n\n%s", usage);
                return 1;
            }
            break;
        case 'C':
        ...
        case 'h':
            ...
            return 0;
        case '?':
            fprintf(stderr, "litv: invalid flag -%c\n\n%s", optopt, usage);
            return 1;
        }
    }
    ...
    if (argc == optind) {
        using_history();
        while (true) {
            char *input = readline("litv> ");
            if (input == NULL) {
                break;
            }
            add_history(input);
            litv_ref *ref = litv_newref();
            int success = litv_parseref(ref, input);
            free(input);
            if (success == 0) {
                litv_render(ref, &config);
            }
            litv_freeref(ref);
        }
    } else {
        ...
        litv_ref *ref = litv_newref();
        int success = litv_parseref(ref, ref_str);
        free(ref_str);
        if (success == 0) {
            litv_render(ref, &config);
        }
